...
function comparisonsForVenues(venues, comparisons) {
...
function buildStackRankings(venues, comparisons) {
  const indexedVenues = venues.map((venue, stackIndex) => ({ ...venue, stackIndex }));
  const usableComparisons = comparisonsForVenues(indexedVenues, comparisons);
  ...
  const counts = comparisonCountsByVenue(usableComparisons);

  let rank = 0;

  return computeRankings(indexedVenues, usableComparisons)
    .map((venue) => {
