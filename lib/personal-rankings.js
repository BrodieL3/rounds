...
function buildStackRankings(venues, comparisons, options = {}) {
  ...
  // doesn't earn a venue a rank), but too-tough draws DO flow into the Elo so the
  // ratings reflect them (ADR 008 §1).
  ...
  const eloOptions = sentimentByVenue ? { seedBySentiment: sentimentByVenue } : {};
  const ranked = computeRankings(indexedVenues, usableComparisons, eloOptions);

  // With sentiment known, present in band-clamped 0–10 display order (loved above
  // fine above disliked); otherwise keep the raw Elo order + raw personalScore.
