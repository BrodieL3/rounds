...
function getFirebaseDeps(deps = {}) {
  ...
  return {
    ...
    addDoc: deps.addDoc || firestoreApi.addDoc,
    collection: deps.collection || firestoreApi.collection,
...
async function saveReviewMediaUrlsAsync(reviewId, mediaUrls, deps = {}) {
  const api = getFirebaseDeps(deps);
  await api.setDoc(
    api.doc(api.db, 'reviews', reviewId),
    { mediaUrls, updatedAt: api.serverTimestamp() },
    { merge: true }
  );
}

async function uploadReviewPhotosAsync(reviewId, localUris = [], deps = {}) {
  if (!reviewId) return { success: false, error: 'reviewId is required', urls: [] };
  if (!Array.isArray(localUris)) return { success: false, error: 'localUris must be an array', urls: [] };
  ...
  try {
    for (let index = 0; index < localUris.length; index += 1) {
      const path = getReviewPhotoPath(reviewId, timestamp, index);
      ...
      const upload = await uploadImageAsync(prepared.uri, path, {
        ...deps,
        maxBytes: deps.maxBytes || MAX_REVIEW_IMAGE_BYTES,
      });
      if (!upload.success) {
    ...
    await saveReviewMediaUrlsAsync(reviewId, urls, deps);
    return { success: true, urls, paths };
  } catch (error) {
    return { success: false, error: normalizeUploadError(error), urls, paths };
  }
}

async function createReviewWithMediaAsync(reviewData, localUris = [], deps = {}) {
  const api = getFirebaseDeps(deps);

  try {
    const reviewRef = await api.addDoc(api.collection(api.db, 'reviews'), {
      ...reviewData,
      mediaUrls: [],
      createdAt: reviewData.createdAt || api.serverTimestamp(),
      updatedAt: api.serverTimestamp(),
    });

    if (localUris.length === 0) {
      return { success: true, reviewId: reviewRef.id, urls: [], paths: [] };
    }

    const upload = await uploadReviewPhotosAsync(reviewRef.id, localUris, deps);
    if (!upload.success) return { ...upload, reviewId: reviewRef.id };

    return { success: true, reviewId: reviewRef.id, urls: upload.urls, paths: upload.paths };
  } catch (error) {
    return { success: false, error: normalizeUploadError(error), urls: [] };
  }
}
