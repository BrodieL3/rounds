...
function buildRatingPayload({
  ...
  // Location is the VENUE's, not the user's (ADR 007). The venue (from the
  // bundled seed) carries an authoritative city; copy it onto the Rating and
  // derive the metro lens from it. profile.city is only a legacy fallback for
  // venues that predate geocoding — never a city the user merely lives in.
  const city = venue.city || profile?.city || null;
  const payload = {
    userId: user.uid,
...
function buildPostProjection({ ratingId, rating } = {}) {
  ...
  const projection = {
    ...
    city: rating.city,
    metro: rating.metro ?? getMetroForCity(rating.city),
    visibility: 'public',
    likes: 0,
    likedBy: [],
    bookmarks: 0,
