 ...
 * @param {{ displayName:string, username:string, dob:string }} fields
 * @param {{ serverTimestamp: () => unknown }} deps
 */
function buildOnboardingProfile({ displayName, username, dob }, { serverTimestamp }) {
  return {
    displayName,
    username,
    dateOfBirth: dob,
...
async function persistOnboarding(input, api) {
  ...
  try {
    ...
    const profile = buildOnboardingProfile(
      { displayName: validation.displayName, username: validation.username, dob: input.dob },
      api,
    );
