...
async function sendDirectTextMessage({ db, senderUid, recipientUid, text }, api = defaultApi()) {
  const conversationId = buildDirectConversationId(senderUid, recipientUid);
...
function sortConversationsByLastMessage(a, b) {
  return (toComparableTime(b.lastMessageAt) || 0) - (toComparableTime(a.lastMessageAt) || 0);
}
...
async function buildInboxConversation({ db, uid, conversation, api }) {
  ...
  const otherUid = getOtherMemberUid(conversation, uid);
  const otherSnap = otherUid ? await api.getDoc(api.doc(db, 'users', otherUid)) : null;
  const otherUser = docData(otherSnap);
  const displayName = otherUser?.displayName || otherUser?.username || otherUid || 'Conversation';
  ...
  return {
    ...
    preview: formatInboxMessagePreview(conversation.lastMessage || {}, {
      ...
      senderName: conversation.lastMessage?.senderUid === uid ? 'You' : displayName,
    }),
  };
}
...
function subscribeConversationMessages({ db, conversationId, onChange, onError }, api = defaultApi()) {
  return api.onSnapshot(api.collection(db, 'conversations', conversationId, 'messages'), (snapshot) => {
