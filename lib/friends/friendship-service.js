...
const { buildBlockId, buildFriendRequestId, buildFriendshipId, getSortedPairUids } = require('./ids');
...
async function loadFriendshipStatus({ db, viewerUid, otherUid }, api = defaultApi()) {
  ...
  const viewerBlockId = buildBlockId(viewerUid, otherUid);
  const otherBlockId = buildBlockId(otherUid, viewerUid);

  const [outgoingSnap, incomingSnap, friendshipSnap, viewerBlockSnap, otherBlockSnap] = await Promise.all([
    ...
    api.getDoc(api.doc(db, 'blocks', viewerBlockId)),
    api.getDoc(api.doc(db, 'blocks', otherBlockId)),
  ]);
  ...
  return deriveFriendshipStatus({
    ...
    blocks: [docData(viewerBlockSnap), docData(otherBlockSnap)].filter(Boolean),
  });
}
