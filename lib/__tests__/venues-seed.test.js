...
describe('venue seed data (Overture / Foursquare OS Places)', () => {
  test('uses Overture source metadata and exactly Boston/Cambridge city keys', () => {
    expect(venueSeed).toHaveProperty('cities');
    expect(venueSeed.attribution).toEqual(expect.stringContaining('Overture'));
    expect(venueSeed.source).toBe('overture-places');
