...
describe('Friendship slice UI wiring', () => {
  ...
  test('Friend requests page subscribes to incoming pending requests and supports accept/decline', () => {
    const source = fs.readFileSync(path.join(__dirname, '..', '..', 'app', 'friend-requests.js'), 'utf8');
  ...
  test('Friends tab surfaces pending requests and links to the friend requests page', () => {
    const source = fs.readFileSync(path.join(__dirname, '..', '..', 'app', '(tabs)', 'friends.js'), 'utf8');

    expect(source).toContain('subscribeIncomingFriendRequests');
    expect(source).toContain('/friend-requests');
  });
});
