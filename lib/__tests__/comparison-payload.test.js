...
describe('comparison-log support', () => {
  ...
  test('VALID_CONTEXTS enumerates the comparison intents (outcome stays in result)', () => {
    expect(VALID_CONTEXTS).toEqual([
      'pairwise', 'placement-search', 'organic-rerank', 'tonight-decision',
    ]);
  });

  test('accepts a tonight-decision comparison (ADR 010 §5)', () => {
    const fields = {
      userId: 'u1', cohort: 'cocktail_bar', venueA: 'vA', venueB: 'vB', result: 'vA',
      sessionId: 's1', sequence: 0, createdAt: 123, context: 'tonight-decision',
    };
    expect(() => buildComparisonPayload(fields)).not.toThrow();
    expect(buildComparisonPayload(fields).context).toBe('tonight-decision');
  });
});
