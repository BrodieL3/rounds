...
describe('Figma UI overhaul route shell contract', () => {
  ...
  test('keeps Plus formSheet inside app shell and registers the on-ramp onboarding route (F1 slice 2)', () => {
    expect(getRouteMetadata('plus-menu')).not.toBeNull();
    expect(getRootStackScreens().map((screen) => screen.name)).toContain('plus-menu');
    // F1 slice 2 reintroduces the auth/onboarding on-ramp: onboarding is now a
    // registered root-stack route (parent ISA ISC-2/4), reversing the earlier
    // Figma-pass assertion that the shell had no onboarding dependency.
    expect(getRootStackScreens().map((screen) => screen.name)).toContain('onboarding');
    expect(getRouteMetadata('onboarding')).not.toBeNull();
  });

  test('documents transparentModal decision for the center Plus sheet', () => {
