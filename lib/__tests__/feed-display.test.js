...
describe('feed display helpers', () => {
  test('builds native timeline copy from a city post', () => {
    ...
    expect(display.engagement).toEqual({ likes: '12 likes', bookmarks: '2 saved reviews' });
  });

  test('uses the post city for the area label, never the viewer city', () => {
    const display = buildFeedItemDisplay({
      id: 'post-2',
      displayName: 'Bay',
    ...
    // Viewer is 'nyc', but a Boston review must read "Boston", never "New York".
    expect(display.metadata).toBe('🍷 Wine Bar · Boston · 1h ago');
    expect(display.metadata).not.toMatch(/New York/);
  });

  test('combines a precise neighborhood with the post city', () => {
    const display = buildFeedItemDisplay({
      id: 'post-3',
      venueName: 'Lord Hobo',
      cohort: 'pub',
      ...
      createdAt: new Date('2026-05-29T11:00:00Z'),
    }, 'boston', new Date('2026-05-29T12:00:00Z'));

    expect(display.metadata).toBe('🍺 Pub · Inman Square, Cambridge · 1h ago');
  });
