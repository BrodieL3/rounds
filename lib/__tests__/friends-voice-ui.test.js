...
describe('Voice attachment UI wiring', () => {
  ...
  test('VoicePanel records on press-and-hold with an animated red fill', () => {
    const panel = read('components', 'conversation', 'panels', 'VoicePanel.js');
    const hook = read('hooks', 'useConversationSurface.js');

    expect(panel).toContain('onPressIn');
    expect(panel).toContain('onPressOut');
    expect(panel).toContain('Animated'); // slow red fill out from the mic icon
