...
describe('buildEventPayload', () => {
  ...
  test('accepts the venue seed location block when top-level coordinates are absent', () => {
    const locationOnlyVenue = {
      id: 'ovt:location-only',
      name: 'Map Room',
      cohort: 'cocktail_bar',
      city: 'cambridge',
    ...
    const p = buildEventPayload({ ...base, venue: locationOnlyVenue });
    expect(p.lat).toBe(42.3736);
    expect(p.lng).toBe(-71.1097);
  });
  ...
  test('preserves source richness from real event providers', () => {
    const p = buildEventPayload({
      ...
      minAge: 21,
      isFree: false,
      organizerName: 'Resident Advisor',
    });

    expect(p.sourceEventId).toBe('123');
    expect(p.lineup).toEqual(['Ellen Allien']);
    expect(p.genres).toEqual(['Techno', 'Acid']);
    expect(p.price).toBe('$25');
    expect(p.minAge).toBe(21);
    expect(p.isFree).toBe(false);
    expect(p.organizerName).toBe('Resident Advisor');
  });
