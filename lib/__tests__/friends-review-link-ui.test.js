...
describe('Review link UI wiring', () => {
  ...
  test('MessageBubble renders review_link cards', () => {
    const bubble = read('components', 'conversation', 'MessageBubble.js');
    expect(bubble).toContain("const isReviewLink = message.type === 'review_link';");
    expect(bubble).toContain('styles.reviewLinkCard');
    expect(bubble).toContain("pathname: '/post/[id]'");
  });
