...
// posts and ZERO friends must FIND a bar to log. The Discover tab must show the
// browsable seeded catalog (not only the empty `posts` feed), and every row
// must navigate to the venue detail where slice-1's "Log a visit" already works.
// No JSX/RTL transform in this repo → source-contract assertions, matching
// venue-detail-ui.test.js / log-visit-loop-ui.test.js.

describe('Discover — browsable seeded catalog (cold-start fix)', () => {
  const source = read('app', '(tabs)', 'discover.js');
  ...
  test('does NOT gate the catalog on profile.city (a new user has no city set)', () => {
    ...
    const catalogBuild = source.indexOf('buildVenueCatalog(venueSeed)');
    expect(catalogBuild).toBeGreaterThan(-1);
    // The catalog memo depends only on the static seed, never on profile.city.
    expect(source).toContain('useMemo(() => buildVenueCatalog(venueSeed), [])');
  });
  ...
  test('exposes a path into search so users can also find by name', () => {
