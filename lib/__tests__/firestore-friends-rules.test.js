...
describeRules('Friends Firestore rules', () => {
  test('signed-in user can run beta UI list queries for empty social state', async () => {
    const aliceDb = dbFor('alice');

    await assertSucceeds(getDocs(query(
      collection(aliceDb, 'friendRequests'),
      where('toUid', '==', 'alice'),
    ...
    await assertSucceeds(getDocs(query(
      collection(aliceDb, 'friendships'),
      where('memberUids', 'array-contains', 'alice'),
    )));
    await assertSucceeds(getDocs(query(
      collection(aliceDb, 'comparisons'),
      where('userId', '==', 'alice'),
    )));
    await assertSucceeds(getDocs(collection(aliceDb, 'users', 'alice', 'venueBookmarks')));
  });
  ...
  test('member can send a reply quote text message but lastMessage stays plain text', async () => {
    await seed(['conversations', 'dm_alice_bob'], {
      type: 'dm',
      memberUids: ['alice', 'bob'],
      createdAt: 3,
    ...
    batch.set(doc(bobDb, 'conversations', 'dm_alice_bob', 'messages', 'm2'), {
      ...
      text: 'omw',
      replyToMessageId: 'm1',
      replyToPreview: { senderUid: 'alice', type: 'text', snippet: 'meet at 9?' },
      createdAt: 4,
      deletedForEveryoneAt: null,
    });
    batch.set(doc(bobDb, 'users', 'bob', 'conversationStates', 'dm_alice_bob'), {
    ...
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'badReply'), {
      senderUid: 'bob',
      type: 'text',
      text: 'bad',
      replyToMessageId: 'm1',
      replyToPreview: { senderUid: 'alice', type: 'text', snippet: 'x'.repeat(241) },
      createdAt: 5,
      deletedForEveryoneAt: null,
    }));
  });

  test('conversation members can read and toggle their own reactions only', async () => {
    await seed(['conversations', 'dm_alice_bob'], {
      type: 'dm',
      memberUids: ['alice', 'bob'],
      createdAt: 3,
    ...
    await seed(['conversations', 'dm_alice_bob', 'messages', 'm1'], {
      senderUid: 'alice', type: 'text', text: 'meet at 9?', createdAt: 3, deletedForEveryoneAt: null,
    });

    await assertSucceeds(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob'), {
      uid: 'bob', emoji: '👍', createdAt: 4,
    }));
    await assertSucceeds(updateDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob'), {
      uid: 'bob', emoji: '🔥', createdAt: 5,
    }));
    await assertSucceeds(getDoc(doc(dbFor('alice'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob')));
    await assertSucceeds(deleteDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob')));

    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'alice'), {
      uid: 'alice', emoji: '👍', createdAt: 4,
    }));
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob'), {
      uid: 'bob', emoji: '🚀', createdAt: 4,
    }));
    await assertFails(getDoc(doc(dbFor('cara'), 'conversations', 'dm_alice_bob', 'messages', 'm1', 'reactions', 'bob')));
  });
  ...
  test('user can hide, hide messages, and mark own conversation seen but cannot update another user state', async () => {
    ...
    // Pinning writes pinnedAt to the user's own state (regression: previously rejected by hasOnly).
    await assertSucceeds(setDoc(doc(dbFor('alice'), 'users', 'alice', 'conversationStates', 'dm_alice_bob'), {
      hiddenAt: null, lastSeenAt: 5, pinnedAt: 6,
    }));
    // Unknown fields are still rejected.
    await assertFails(setDoc(doc(dbFor('alice'), 'users', 'alice', 'conversationStates', 'dm_alice_bob'), {
      bogusField: 7,
    }));
  });
  ...
  test('location message shape is validated as part of validMessageShape', async () => {
    await seed(['users', 'alice'], { uid: 'alice', followers: ['bob'], following: ['bob'] });
    await seed(['users', 'bob'], { uid: 'bob', followers: ['alice'], following: ['alice'] });
    await seed(['friendships', 'alice_bob'], { memberUids: ['alice', 'bob'], createdAt: 1 });
    await seed(['conversations', 'dm_alice_bob'], {
    ...
    await assertSucceeds(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'loc1'), {
      senderUid: 'bob', type: 'location', lat: 40.7128, lng: -74.0060,
      label: 'Good Bar', createdAt: 4, deletedForEveryoneAt: null,
    }));

    // Missing lat
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'bad'), {
      senderUid: 'bob', type: 'location', lng: -74.0060,
      label: 'Good Bar', createdAt: 5, deletedForEveryoneAt: null,
    }));

    // Invalid label length
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'bad'), {
      senderUid: 'bob', type: 'location', lat: 0, lng: 0,
      label: 'x'.repeat(241), createdAt: 5, deletedForEveryoneAt: null,
    }));
  });

  test('voice message shape is validated as part of validMessageShape', async () => {
    await seed(['users', 'alice'], { uid: 'alice', followers: ['bob'], following: ['bob'] });
    await seed(['users', 'bob'], { uid: 'bob', followers: ['alice'], following: ['alice'] });
    await seed(['friendships', 'alice_bob'], { memberUids: ['alice', 'bob'], createdAt: 1 });
    await seed(['conversations', 'dm_alice_bob'], {
    ...
    // Valid voice message
    await assertSucceeds(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'v1'), {
      senderUid: 'bob', type: 'voice',
      storagePath: 'conversations/dm_alice_bob/voice/voice_1.m4a',
      durationMs: 15000, format: 'm4a', savedBy: [],
      expiresAt: Date.now() + 24 * 60 * 60 * 1000,
      createdAt: 4, deletedForEveryoneAt: null,
    }));
    ...
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'bad'), {
      senderUid: 'bob', type: 'voice',
      storagePath: 'conversations/dm_alice_bob/voice/voice_1.m4a',
      durationMs: 70000, format: 'm4a', savedBy: [],
      expiresAt: Date.now() + 24 * 60 * 60 * 1000,
      createdAt: 5, deletedForEveryoneAt: null,
    }));
    ...
    await assertFails(setDoc(doc(dbFor('bob'), 'conversations', 'dm_alice_bob', 'messages', 'bad'), {
      senderUid: 'bob', type: 'voice',
      storagePath: 'conversations/dm_alice_bob/voice/voice_1.m4a',
      durationMs: 15000, format: 'mp3', savedBy: [],
      expiresAt: Date.now() + 24 * 60 * 60 * 1000,
      createdAt: 5, deletedForEveryoneAt: null,
    }));
  });
