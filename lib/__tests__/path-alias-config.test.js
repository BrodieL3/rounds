...
describe('Expo standard path aliases', () => {
  ...
  test('entry route redirects straight into rebuilt Figma shell', () => {
    const indexRoute = readRoot('app/index.js');

    expect(indexRoute).toContain("import { Redirect } from 'expo-router'");
    expect(indexRoute).toContain('<Redirect href="/(tabs)/friends" />');
    expect(indexRoute).not.toContain('AuthContext');
    expect(indexRoute).not.toContain('auth-routing');
  });
});
