...
Current slice in development: none. Last completed slice: Architecture hardening 1 — deepen Conversation Message send. Message interactions slice. The original Friends-first implementation track is complete through text DMs, group chats, review companion tags, venue/review links, unlisted Rating shares, polls, photos, location pins, voice notes, safety basics, and message reactions/reply quotes.
...
Architecture hardening 1 completion note: Deepened Conversation Message send into `lib/friends/message-send-service.js`. The deep module owns direct vs group write flow, first DM creation, existing DM update, group update, message doc writes, sender/recipient conversation states, notification fanout, `lastMessageAt`/`lastMessage`, injectable message id/Firestore API for tests, optional upload-before-batch hook for media messages, and best-effort cleanup hook when upload succeeds but batch fails. Message-type modules (`dm-service`, `group-service`, `photo-service`, `poll-service`, `location-service`, `voice-service`, `venue-link-service`, `review-link-service`) are now thin adapters that build message payload/lastMessage only. All existing public exports preserved; no UI churn. Firestore doc shapes unchanged; rules unchanged. Verified with `npm test -- --runInBand` (61 suites passed, 2 skipped; 292 tests passed, 49 skipped), `npm run test:rules` (3 suites passed; 54 tests passed), and `npx expo export --platform web` (Exported: dist).
