...
Started media adapter fast-follow: added SDK-compatible `expo-image` dependency, created `components/ui/media-image.js` with shared Expo Image defaults, added `lib/media-image-source.js` URI normalization helper, and migrated Feed avatars/review media plus Conversation single/grid photo bubbles to the adapter. Verified with `npm test -- --runInBand` and `npx expo export --platform web --output-dir /tmp/rounds-web-export-native-ui-media-conversation`.
